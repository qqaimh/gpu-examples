...
@Component({
  selector: 'gpu-examples-gpu-examples-sampler-parameters',
  templateUrl: './gpu-examples-sampler-parameters.component.html',
  styleUrls: ['./gpu-examples-sampler-parameters.component.scss']
})
export class GpuExamplesSamplerParametersComponent implements OnInit {
  ...
  async draw() {
...

    {
      ...
      const flangeFold = this.gui.addFolder('Plane settings');
      flangeFold.open();
      ...
      this.gui.width = 280;
      {
        const folder = this.gui.addFolder('GPUSamplerDescriptor');
        folder.open();
    ...
    const kCanvasLayoutCSSSize = 800; // set by template styles
    const kCanvasLayoutDevicePixels = kCanvasLayoutCSSSize * this.devicePixelRatio;
    const kScaleFactor = Math.floor(kCanvasLayoutDevicePixels / kCanvasSize);
    const kCanvasDevicePixels = kScaleFactor * kCanvasSize;
    const kCanvasCSSSize = kCanvasDevicePixels / this.devicePixelRatio;
    console.log(kCanvasDevicePixels, kCanvasCSSSize);
    this.theCanvas.nativeElement.style.imageRendering = 'pixelated';
    this.theCanvas.nativeElement.width = this.theCanvas.nativeElement.height = kCanvasSize;
    this.theCanvas.nativeElement.style.minWidth = this.theCanvas.nativeElement.style.maxWidth = kCanvasCSSSize + 'px';
