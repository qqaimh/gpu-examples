...
import vertexDepthPrePassWGSL from './shaders/vertexDepthPrePass.wgsl';
import vertexTextureQuadWGSL from './shaders/vertexTextureQuad.wgsl';
import fragmentTextureQuadWGSL from './shaders/fragmentTextureQuad.wgsl';
import vertexPrecisionErrorPassWGSL from './shaders/vertexPrecisionErrorPass.wgsl';
import fragmentPrecisionErrorPassWGSL from './shaders/fragmentPrecisionErrorPass.wgsl';
...
@Component({
  selector: 'app-gpu-examples-reversed-z',
  templateUrl: './gpu-examples-reversed-z.component.html',
  styleUrls: ['./gpu-examples-reversed-z.component.scss']
})
export class GpuExamplesReversedZComponent implements OnInit {
  ...
  // This example shows the use of reversed z technique for better utilization of depth buffer precision. The left column uses regular method, 
  // while the right one uses reversed z technique. Both are using depth32float as their depth buffer format. A set of red and green planes are positioned very close to each other. 
  // Higher sets are placed further from camera (and are scaled for better visual purpose). To use reversed z to render your scene, you will need depth store value to be 0.0, 
  // depth compare function to be greater, and remap depth range by multiplying an additional matrix to your projection matrix. 
  // Related reading: 
  ...
  async draw() {
    ...
    this.gui.add(settings, 'mode', ['color', 'precision-error', 'depth-texture']);
