...
export class Gltf2Loader {
  ...
  async loadFromJson(json, baseUrl, binaryChunk) {
    ...
    if (!this.#dracoDecoder && json.extensionsUsed.includes('KHR_draco_mesh_compression')) {
      this.#dracoDecoder = new DracoDecoder();
    }
    ...
    function resolveLight(index) {
      ...
      if (!clientLight) {
        const light = Object.assign({}, DEFAULT_LIGHT, KHR_lights_punctual.lights[index]);
        clientLight = Promise.resolve(client.createLight(light, index));
